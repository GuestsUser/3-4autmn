...
void PageOne::PageOne_Update() {
	...
	for (i = 0; i < Player_card.size(); i++) {
		...
		//switch (i) {
		//case 0:
		//	if (Card::Hit(Mouse_X, Mouse_Y, Player_X + (card_w / 2) * i, Player_Y + (card_h / 2) * (i / 10), card_w, card_h)) {
		//		j = GetColor(255, 0, 0);
		//	}
		//	break;
		//case 1:
		...
		if (Card::Hit(Mouse_X, Mouse_Y, Player_X + (card_w / 2) * i, Player_Y * (i / 10), card_w, card_h)) {
	...
	for (i = 0; i < NPC_card_1.size(); i++) {
		if (Card::Hit(Mouse_X, Mouse_Y, 150 + (card_w * 0.3) * i, 75, card_w, card_h)) {
...
void PageOne::PageOne_Draw() {
	...
	//場に出ているカードの描画
	for (auto itr = Field_card.begin(); itr != Field_card.end(); itr++) {
