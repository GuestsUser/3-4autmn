...
void HandComboCheck(std::deque<int> hand, PK_CardDealer::deque_map& get) { //handに現在手札を渡すとその手札で同じ強さのカードの個数をgetに入れてくれる
	for (int i = 0; i < hand.size(); ++i) {
		...
		for (int j = i + 1; j < hand.size(); ++j) {
			if (get[sub].first != hand[j] % (int)PK_CardDealer::CardPower::max) { continue; } //札が違う場合やり直し
...
int HandStraightCheck(const PK_CardDealer::deque_map& combo) { //ストレート(数値が1刻みで並んでる)をチェック、並んでる場合最も大きいカードパワーの返却、失敗の場合-1
	...
	for (auto itr = ++combo.begin(); itr != combo.end(); ++itr) {
		...
		if (old == (int)PK_CardDealer::CardPower::ace && itr->first == (int)PK_CardDealer::CardPower::five) { ato5 = true; old = itr->first; continue; } //前回がAで今回が5ならAを1として扱う為続ける
...
bool HandRoyalCheck(const PK_CardDealer::deque_map& combo) { //ロイヤル(並びがa～10)をチェック
	for (int i = 0; i < combo.size(); ++i) { if (combo[i].first != (int)PK_CardDealer::CardPower::ace - i) { return false; } } //comboは整列済みなので数列に例外的な値があれば失敗
	return true;
}
int HandFlashReachCheck(const std::deque<int>& hand) { //フラッシュリーチ(スート違いが1つだけ)チェック、返り値はスートの違うカード、リーチになってない場合-1
	...
	for (auto itr : hand) { suitVol[(int)(itr / (int)PK_CardDealer::CardPower::max)].push_back(itr); }
...
int HandStraightReachCheck(PK_CardDealer::deque_map combo, const std::deque<int>& hand) { //ストレートリーチ(スート違いが1つだけ)チェック、返り値はスートの違うカード、リーチになってない場合-1
	int straight = -1;
	...
	if (combo[0].first == (int)PK_CardDealer::CardPower::ace && (combo[1].first == (int)PK_CardDealer::CardPower::five || combo[1].first == (int)PK_CardDealer::CardPower::four)) { //ace,twoの並びの場合aceをoneにして配列の最後へ格納する
		int copy = combo[0].second; //先頭のaceのカード枚数を記憶
		combo.erase(combo.begin()); //先頭を削除
		combo.push_back(std::pair<int, int>((int)PK_CardDealer::CardPower::one, copy)); //後ろに追加aceをoneに改めた物を追加
	}
	...
	if (straight == -1) { straight = combo[combo.size() - 1].first == (int)PK_CardDealer::CardPower::one ? (int)PK_CardDealer::CardPower::ace : combo[combo.size() - 1].first; } //交換指定がない場合一番弱いカードを交換する、oneの場合aceに直しておく
	...
	for (auto itr : hand) { //comboはカードをカードパワーで格納してるのでそれを通常カードに直す処理
		if (itr % (int)PK_CardDealer::CardPower::max == straight) { return itr; } //不要なカードとhand内の一致するカードパワーがあればそのカードを不要なカードとして返して終わり
	}
...
std::deque<int> PK_CardDealer::HandCheck(const PK_Chara& chara) {
...
int PK_CardDealer::ReachCheck(const PK_Chara& chara) {
	deque_map combo = deque_map(); //first=カードの強さ second=そのカードの枚数 で構成されたカード格納配列
	std::deque<int> hand; //手札を数値化した物を格納
	chara.GetHandNum(hand); //キャラから手札配列を取得
	HandComboCheck(hand, combo); //同一カードを格納する
	...
	if (flash > -1 && straight > -1) { //ストレート、フラッシュ両方可能だった場合
		if (flash % (int)PK_CardDealer::CardPower::max == straight % (int)PK_CardDealer::CardPower::max) { straight = flash; } //ストレートの変更カードに重複カードがあった場合2つのカードの内hand要素の若い方を返すのだが、flashと要求カードパワーが一緒だったならflashのカードに合わせる
