...
void Poker::ShowDown::Update() {
	...
	if (count == 180) {
		std::deque<PK_Chara*> enableChara = std::deque<PK_Chara*>(); //勝負を行うキャラ纏め
		...
		while (enableChara.size() > 0) { //enableChara全てに支払い終えるまで実行
			...
			while (copy.size() > 0) { //copyが空になるまで続ける
				std::deque<PK_Chara*> same = std::deque<PK_Chara*>(1, enableChara[copy[0]]); //支払額同一のキャラまとめ
			...
			std::deque<PK_Chara*> maxChara = std::deque<PK_Chara*>(); //maxを基にenableCharaから取り出した物を保持する配列
