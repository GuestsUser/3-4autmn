...
PK_Player::PK_Player(Pot& pot, PK_Dealer& dealer, CardDealer& cardDealer) :pot(&pot), dealer(&dealer), cardDealer(&cardDealer), sectionModule(0) {
	...
	actionButton->SetAlways(actionButtonImage); //ボタンに画像を追加
	foldButton->SetAlways(foldImage);
	actionButton->SetClick(new Cmp_Button_ClickCheck()); //クリックにクリック検知用空コンポーネントの追加
	foldButton->SetClick(new Cmp_Button_ClickCheck());
