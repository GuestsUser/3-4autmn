...
void CP::deckshuffle() {
	...
	onesec();
	DrawFormatString(50, 350, GetColor(255, 255, 255), "山札をシャッフルしました");
}
...
void CP::dealcards() {
	...
	onesec();
	DrawFormatString(50, 350, GetColor(255, 255, 255), "手札を配りました");
}
...
void CP::d3start() {
	...
	onesec();
	 DrawFormatString(50, 350, GetColor(255, 255, 255), "ダイヤの３を持ってる人からスタートします");
}
...
void CP::phase(int n) {
	...
	if (n == pc) {
		onesec();
		DrawFormatString(50, 350, GetColor(255, 255, 255), "場が流れました");
		pc = -1;
		eback = 0; /* 11バックをリセット */
...
void CP::status(int n) {
	...
	if (trash[0] == 0) /* ﾄﾗｯｼｭが空だった場合 */
		DrawFormatString(50, 350, GetColor(255, 255, 255), "[場] 空");

	else { /* ﾄﾗｯｼｭにｶｰﾄﾞがあった場合 */
		DrawFormatString(50, 350, GetColor(255, 255, 255), "[場]");
		for (i = 1; i <= trash[0]; i++) {
	...
	for (i = 1; i <= data[n].hand[0]; i++) {
		...
		if (mar(*data[n].hand) != 'J')
		 DrawFormatString(50, 350, GetColor(255, 255, 255), " %c%2d ", mar(*data[n].hand), num(data[n].hand[i]));
		else
		 DrawFormatString(50, 350, GetColor(255, 255, 255), " %c ", mar(data[n].hand[i]));
	}
